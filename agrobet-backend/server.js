...
<body class="bg-gray-100">
    ...
    <script>
        ...
        async function loadOpenGames() {
            try {
                const response = await fetch(`${SERVER_URL}/games`, { credentials: 'include' });
                ...
                games.forEach(game => {
                    const userBetsOnThisGame = currentUserBets.filter(bet => bet.gameId === game._id);
                    const totalAlreadyBet = userBetsOnThisGame.reduce((acc, bet) => acc + bet.betValue, 0);
                    const remainingValue = game.maxBetValue - totalAlreadyBet;

                    let betLimitMessage = `Aposta máxima: R$ ${game.maxBetValue.toFixed(2)}`;
                    if (totalAlreadyBet > 0) {
                        betLimitMessage = `Já apostou R$ ${totalAlreadyBet.toFixed(2)}. Pode apostar mais R$ ${remainingValue.toFixed(2)}.`;
                    }
                        ...
                        <div class="p-6">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="grid grid-cols-3 gap-2 bet-options">
                                    <button class="bet-option py-2 border rounded flex flex-col items-center" data-option="home" ${remainingValue <= 0 ? 'disabled' : ''}><span>${game.home.name}</span><span class="font-bold text-green-600">${game.odds.home.toFixed(2)}</span></button>
                                    <button class="bet-option py-2 border rounded flex flex-col items-center" data-option="empate" ${remainingValue <= 0 ? 'disabled' : ''}><span>EMPATE</span><span class="font-bold text-green-600">${game.odds.draw.toFixed(2)}</span></button>
                                    <button class="bet-option py-2 border rounded flex flex-col items-center" data-option="away" ${remainingValue <= 0 ? 'disabled' : ''}><span>${game.away.name}</span><span class="font-bold text-green-600">${game.odds.away.toFixed(2)}</span></button>
                                </div>
                            </div>
                            <div class="mb-4">
                                <h4 class="font-bold mb-2 text-center text-gray-700">VALOR DA APOSTA (R$):</h4>
                                ${remainingValue > 0 ? `
                                    <input type="number" class="bet-value-input w-full p-2 border rounded text-center" min="1" max="${remainingValue.toFixed(2)}" step="any" placeholder="Ex: 10.00">
                                    <p class="text-xs text-center text-gray-500 mt-1">${betLimitMessage}</p>
                                ` : `
                                    <div class="text-center p-2 bg-gray-100 rounded-md">
                                        <p class="font-bold text-red-600">Limite de aposta atingido!</p>
                                        <p class="text-xs text-gray-600">Já apostou o máximo de R$ ${game.maxBetValue.toFixed(2)} neste jogo.</p>
                                    </div>
                                `}
                            </div>
                            ...
                            <div class="bet-error text-red-500 text-sm text-center h-4 mb-2"></div>
                            <button class="confirm-btn w-full py-3 rounded-md text-white font-bold text-lg" ${remainingValue <= 0 ? 'disabled' : ''}>
                                ${remainingValue > 0 ? 'APOSTAR' : 'LIMITE ATINGIDO'}
                            </button>
                        </div>
                    </div>`;
                });
        ...
        async function loadResults() {
             try {
                const response = await fetch(`${SERVER_URL}/results`, { credentials: 'include' });
        ...
        function addGameEventListeners() {
            document.querySelectorAll('.bet-card').forEach(card => {
                ...
                const errorEl = card.querySelector('.bet-error');

                if (confirmBtn.disabled) return; // Se o botão já estiver desativado, não adicione listeners
                ...
                betOptions.addEventListener('click', e => {
                    ...
                    const oddsKey = selectedOption === 'empate' ? 'draw' : selectedOption;
                    selectedOdd = game.odds[oddsKey] || 0;
                    updatePayout();
                });
                ...
                confirmBtn.addEventListener('click', () => {
                    errorEl.textContent = ''; // Limpa erro anterior
                    const value = parseFloat(valueInput.value);

                    // Validação do lado do cliente
                    if (value > parseFloat(valueInput.max)) {
                        errorEl.textContent = `O valor máximo restante é R$ ${parseFloat(valueInput.max).toFixed(2)}.`;
                        return;
                    }
                    
                    handleBet(gameId, selectedOption, value, card);
                });
            });
        }

        async function handleBet(gameId, option, value, card) {
            const errorEl = card.querySelector('.bet-error');
            if (!currentUser) {
                showError("Por favor, faça login para poder apostar.");
        ...
        async function handleAuthAction() {
            ...
            try {
                ...
                if (data.success) { 
                    ...
                    showError(data.message || 'Ocorreu um erro.'); 
                }
            } catch (err) { 
        ...
        async function initializeUserSession() {
            ...
            try {
                const response = await fetch(`${SERVER_URL}/my-bets/${currentUser.pix}`, { credentials: 'include' });
                const data = await response.json();
                if (data.success) {
                    currentUserBets = data.bets;
                }
            } catch (error) {
                console.error("Erro ao carregar as apostas do utilizador:", error);
                currentUserBets = [];
            }
            await Promise.all([loadOpenGames(), loadResults()]);
        }
        ...
        async function displayMyBets() {
            ...
            // Re-utiliza os dados já carregados
            if (currentUserBets.length > 0) {
                 listElement.innerHTML = currentUserBets.map(bet => `
