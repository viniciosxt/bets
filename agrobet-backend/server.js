...
app.get('/relatorio', async (req, res) => {
    try {
        ...
        bets.forEach(bet => {
            const date = bet.date ? new Date(bet.date).toLocaleString('pt-BR') : 'N/A';
            const userName = bet.user?.name || 'N/A';
            const userPix = bet.user?.pix || 'N/A';
            const gameTitle = bet.gameTitle || 'N/A';
            const betChoice = bet.betChoice || 'N/A';
            const betValue = typeof bet.betValue === 'number' ? `R$ ${bet.betValue.toFixed(2)}` : 'N/A';

            html += `
                <tr class="border-b">
                    <td class="py-3 px-4">${date}</td>
                    <td class="py-3 px-4">${userName}</td>
                    <td class="py-3 px-4">${userPix}</td>
                    <td class="py-3 px-4">${gameTitle}</td>
                    <td class="py-3 px-4">${betChoice}</td>
                    <td class="py-3 px-4 font-semibold text-green-700">${betValue}</td>
                </tr>`;
        });
...
app.get('/admin/games', authAdmin, async (req, res) => {
    try {
        ...
        const gameRows = games.map(game => {
            ...
            const closeButton = `<form action="/admin/close-game/${game._id}" method="post" class="inline-block"><button class="bg-yellow-500 text-white px-3 py-1 rounded text-sm mt-2">Fechar Apostas</button></form>`;
            const finalizeForm = `
            ...
            return `
                <div class="border p-4 rounded-lg">
                    <p class="font-bold text-lg">${game.home.name} vs ${game.away.name} - <span class="text-gray-600">${game.competition || 'N/A'}</span></p>
                    <p>Status: <span class="font-semibold">${game.status}</span> | Resultado: <span class="font-semibold">${game.result}</span></p>
                    ${game.status === 'aberto' ? closeButton : ''}
                    ${game.status === 'fechado' ? finalizeForm : ''}
                </div>`;
        }).join('');
        ...
        res.send(`<!DOCTYPE html><html lang="pt-BR"><head><title>Gerir Jogos</title><script src="https://cdn.tailwindcss.com"></script></head>
            ...
            <div class="bg-white p-6 rounded shadow-md">
                ...
                <div class="space-y-4">${gameRows}</div>
            </div></div></body></html>`
        );
    } catch (error) { 
        console.error("Erro detalhado ao carregar /admin/games:", error);
        res.status(500).send("Erro ao carregar a página de gestão de jogos."); 
    }
});
